...
create table products(id uuid primary key default gen_random_uuid(),category_id uuid references categories on delete set null,subcategory text,name text not null,slug text unique not null,code text,price numeric not null,promo_price numeric,stock int default 0,stock_status text default 'tersedia',badge text,description text,material text,size text,color text,weight text,rating numeric default 0,review_count int default 0,is_featured bool default false,is_active bool default true,main_image text,video_url text,created_at timestamptz default now());
